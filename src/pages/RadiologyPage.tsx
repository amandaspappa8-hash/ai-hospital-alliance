...
export default function RadiologyPage() {
  ...
  const selectedPatientStudy = useMemo(
    () => pacsStudies.find((study) => study.patientId === selectedPatientId),
    [pacsStudies, selectedPatientId]
  )
  ...
  const openEmbeddedViewer = (studyUid?: string) => {
    setViewerStudyUid(studyUid || selectedPatientStudy?.studyInstanceUID || DEMO_STUDY_UID)
  }

  const openExternalViewer = (studyUid?: string) => {
      ...
      <div style={{ display: "grid", gap: 20 }}>
        <div>
          ...
          <div style={{ marginTop: 8, color: "#cbd5e1" }}>
            Embedded OHIF viewer with real PACS study mapping
          </div>
        </div>
              ...
              {selectedPatient ? (
                <div style={{ display: "grid", gap: 16 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    ...
                    <div style={{ gridColumn: "1 / -1" }}>
                      <strong>Mapped Study UID:</strong> {selectedPatientStudy?.studyInstanceUID ?? "No mapped study"}
                    </div>
                    <div style={{ gridColumn: "1 / -1" }}>
                      <strong>Study Description:</strong> {selectedPatientStudy?.studyDescription ?? "-"}
                    </div>
                  </div>

                  <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                    <button type="button" style={primaryBtn} onClick={() => openEmbeddedViewer()}>
                      Open Embedded OHIF
                    </button>

                    <button type="button" style={secondaryBtn} onClick={() => openExternalViewer()}>
                      Open in New Tab
                    </button>
                  </div>
                </div>
              ...
              >
                <div>
                  ...
                  <div style={{ color: "#cbd5e1" }}>
                    Embedded radiology viewer inside the hospital system
                  </div>
                </div>

                <button type="button" style={secondaryBtn} onClick={() => openExternalViewer()}>
                  External Backup Viewer
                </button>
              </div>

              <iframe title="OHIF PACS Viewer" src={ohifViewerUrl} style={iframeStyle} />
            </div>
                  ...
                  filteredOrders.map((order) => (
                    <div key={order.id} style={itemStyle}>
                      ...
                      <div style={{ marginTop: 8, color: "#93c5fd" }}>
                        Study UID: {order.studyUid ?? "Not assigned"}
                      </div>
                      ...
                      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 12 }}>
                        <button
                          ...
                          onClick={() => openEmbeddedViewer(order.studyUid)}
                        >
                          Open Inside Page
                        </button>
                        ...
                        <button
                          type="button"
                          style={miniSecondaryBtn}
                          onClick={() => openExternalViewer(order.studyUid)}
                        >
                          Open External
                        </button>
                      </div>
