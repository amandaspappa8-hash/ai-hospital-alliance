...
export default function CardiologyPage() {
  ...
  useEffect(() => {
    apiGet<Doctor[]>("/doctors/summary")
      .then(d => setDoctors((Array.isArray(d) ? d : []).filter((doc: Doctor) => doc.specialty === "Cardiology")))
      .catch(() => setDoctors([]))
  }, [])
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#020817,#0f1629)", padding: "28px 32px", fontFamily: "Inter,Arial,sans-serif", color: "white" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <h1 style={{ margin: 0, fontSize: 28, fontWeight: 900, color: "white" }}>{meta.icon} Cardiology</h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
          <div style={{ background: "linear-gradient(135deg,#0f172a,#1a2540)", border: `1px solid ${meta.color}22`, borderRadius: 20, padding: 22 }}>
            <div style={{ fontWeight: 800, fontSize: 14, color: "white", marginBottom: 16 }}>👨‍⚕️ Cardiology Specialists</div>
            {doctors.length === 0 ? (
              <div style={{ color: "#64748b", fontSize: 13 }}>No Cardiology specialists found.</div>
            ) : doctors.map(d => (
