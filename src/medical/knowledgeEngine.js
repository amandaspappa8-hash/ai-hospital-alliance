...
export function knowledgeEngine(input, vitals) {
    ...
    const matches = DISEASE_DB
        ...
        const matchedSymptoms = disease.symptoms.filter((symptom) => text.includes(symptom.toLowerCase())).length;
        const confidence = Math.min(100, Math.round((matchedSymptoms / totalSymptoms) * 100));
        return {
            disease,
            matchedSymptoms,
            confidence,
