...
export const AI_DRUG_FORMULARY: FormularyDrug[] = [
  {
    name: "Aspirin 81 mg",
    generic: "aspirin",
...
export function searchDrugFormulary(query: string) {
  const q = query.trim().toLowerCase()
  if (!q) return []
  return AI_DRUG_FORMULARY.filter(
    (drug) =>
...
export function getDrugCounselingFromFormulary(name: string) {
  const q = name.trim().toLowerCase()
  return AI_DRUG_FORMULARY.find(
    (drug) =>
      drug.name.toLowerCase() === q ||
      drug.generic.toLowerCase() === q
  ) || null
}
