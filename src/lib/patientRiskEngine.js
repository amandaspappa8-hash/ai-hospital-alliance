export function calculatePatientRisk(input) {
    ...
    if (input.hasUnsafeCombination)
        score += 20;
    if (input.missingOnDischarge)
        score += 25;
    if ((input.age || 0) >= 65)
        score += 10;
    if (score >= 90) {
        return {
            ...
            recommendation: "Immediate physician/pharmacist escalation and consider monitored admission or ICU-level review.",
        };
    }
    if (score >= 60) {
        return {
            ...
            recommendation: "Urgent pharmacist and physician review required before discharge.",
        };
    }
    if (score >= 30) {
        return {
            ...
            recommendation: "Close review recommended with discharge verification.",
        };
    }
    return {
        score,
