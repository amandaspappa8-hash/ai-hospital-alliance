...
def patient_to_fhir(patient) -> dict:
    ...
    return {
        ...
        "identifier": [{"system": "http://aiha.hospital/patients", "value": patient.id}],
        "active": patient.status != "Discharged",
        "name": [{"use": "official", "text": patient.name, "family": patient.name.split()[-1], "given": [patient.name.split()[0]]}],
        "gender": patient.gender.lower(),
        "telecom": [{"system": "phone", "value": patient.phone, "use": "mobile"}],
        "extension": [
            {"url": "http://aiha.hospital/fhir/condition", "valueString": patient.condition},
            {"url": "http://aiha.hospital/fhir/department", "valueString": str(patient.department_id)},
        ]
    }
    ...
    return {
        ...
        "component": [
            {"code": {"coding": [{"system": "http://loinc.org", "code": "8867-4", "display": "Heart rate"}]},
             "valueQuantity": {"value": float(vital.heart_rate), "unit": "beats/min", "system": "http://unitsofmeasure.org", "code": "/min"}},
            {"code": {"coding": [{"system": "http://loinc.org", "code": "2708-6", "display": "Oxygen saturation"}]},
             "valueQuantity": {"value": float(vital.oxygen_saturation), "unit": "%", "system": "http://unitsofmeasure.org", "code": "%"}},
            {"code": {"coding": [{"system": "http://loinc.org", "code": "8310-5", "display": "Body temperature"}]},
             "valueQuantity": {"value": float(vital.temperature), "unit": "Cel", "system": "http://unitsofmeasure.org", "code": "Cel"}},
        ]
    }
...
def bundle_response(resources: list, bundle_type: str = "searchset") -> dict:
    ...
    return {
        ...
        "entry": [{"resource": r, "fullUrl": f"http://aiha.hospital/fhir/{r['resourceType']}/{r['id']}"} for r in resources]
    }
