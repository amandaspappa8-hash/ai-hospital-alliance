...
def evaluate_shortness_of_breath(payload: Dict[str, Any]) -> Dict[str, Any]:
    chief = (payload.get("chief_complaint") or "").strip().lower()
    symptoms = set(normalize_text(payload.get("symptoms", [])))
    age = payload.get("age")
    vitals = payload.get("vitals") or {}
    ...
    if age and age >= 60:
        score += 1
        rationale.append("Older age increases respiratory risk context.")
    ...
    if spo2 and spo2 < 92:
        score += 4
        rationale.append("Significant oxygen desaturation detected.")
        red_flags.append("severe low oxygen saturation")
