...
def _build_reconciliation_summary(patient_id: str):
    raw = MED_RECONCILIATION.get(patient_id, {
        "homeMeds": [],
        "admissionMeds": [],
        "dischargeMeds": [],
    })
...
def _default_take_time(med: str):
    ...
    if "aspirin" in med:
        return "After food, once daily in the morning unless your doctor changed the time."
    ...
    if "warfarin" in med:
        return "At the same time every day. Follow INR monitoring instructions carefully."
    if "ibuprofen" in med:
...
def pharmacist_review_mar_item(patient_id: str, item_id: int, payload: PharmacistReviewRequest):
    items = MAR.get(patient_id, [])
    for item in items:
        if item["id"] == item_id:
            item["status"] = payload.status or item.get("status", "Pending")
            ...
            "results": [
                {
                    ...
                    "warnings": [f"Could not fetch live FDA data for {drug_name}. Showing local fallback."],
...
def update_mar_status(patient_id: str, index: int, payload: dict, current_user: dict = Depends(get_current_user)):
    ...
    MAR[patient_id][index]["status"] = payload.get("status", MAR[patient_id][index].get("status"))
    return {"status": "ok", "mar": MAR[patient_id]}


@app.post("/mar/{patient_id}/{index}/pharmacy-review")
def pharmacy_review_mar_item(patient_id: str, index: int, payload: dict, current_user: dict = Depends(get_current_user)):
    if patient_id not in MAR:
        return {"error": "Patient MAR not found"}
    ...
    MAR[patient_id][index]["pharmacy_review"] = payload.get("pharmacy_review", "Reviewed")
...
def add_radiology(patient_id: str, payload: dict, current_user: dict = Depends(get_current_user)):
...
def ai_3d(payload: dict):
    ...
    return {
        "status": "ok",
        "message": "3D segmentation complete",
        "mask": mask
    }
