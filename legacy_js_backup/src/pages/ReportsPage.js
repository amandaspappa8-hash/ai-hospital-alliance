            ...
            try {
                ...
                if (rawDraft) {
                    try {
                        ...
                        const currentPatient = await apiGet(`/patients/${selectedPatientId}`);
                        const currentNotes = await apiGet(`/notes/${selectedPatientId}`);
                        const currentOrders = await apiGet(`/orders/${selectedPatientId}`);
                        setPatient(currentPatient);
                        setNotes(Array.isArray(currentNotes) ? currentNotes : []);
                        setOrders(Array.isArray(currentOrders) ? currentOrders : []);
                        const currentVitals = getMockVitalSigns(currentPatient);
                        const currentImpression = buildClinicalImpression(currentPatient, Array.isArray(currentNotes) ? currentNotes : [], Array.isArray(currentOrders) ? currentOrders : []);
                        const currentRecommendations = buildRecommendations(currentPatient, Array.isArray(currentOrders) ? currentOrders : []);
                        ...
                        setReport({
                            ...
                            status: parsed.status ?? "official-draft",
                ...
                setReport({
                    ...
                    title: "Official Clinical Summary",
                    status: "draft",
                    body: "Failed to generate report from API data.",
                    hospitalName: "AI Hospital Alliance",
                    doctorName: "Dr. Mohammed Al-Fallah, PharmD",
                    visitDate: new Date().toLocaleString(),
                });
